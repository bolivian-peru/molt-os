...
function normalizeCmd(c: string) { return c.replace(/\s+/g, " ").trim().toLowerCase(); }

// App management helpers
const APP_REGISTRY = "/var/lib/osmoda/apps/registry.json";
const APP_PREFIX = "osmoda-app-";

function loadApps(): Record<string, any> {
  try { return JSON.parse(fs.readFileSync(APP_REGISTRY, "utf8")).apps || {}; } catch { return {}; }
}

function saveApps(apps: Record<string, any>) {
  ...
  return [
    ...
    },
    {
      ...
      handler: async (p) => {
        ...
        if (p.actor) qs.set("actor", String(p.actor));
    ...
    // ═══════════════════════════════════════════════════════════════
    {
      ...
      handler: async (p) => {
        ...
        while (shellTimestamps.length > 0 && shellTimestamps[0]! < now - SHELL_WINDOW) shellTimestamps.shift();
        if (shellTimestamps.length > SHELL_RATE_LIMIT) return JSON.stringify({ error: "Rate limit: max 30 shell_exec/min" });
        // Dangerous command check
        const norm = normalizeCmd(cmd);
        if (DANGEROUS.some(d => norm.includes(d))) return JSON.stringify({ error: `Blocked: dangerous command pattern detected in '${cmd.slice(0, 80)}'` });
        // Execute
    ...
    },
    {
      ...
      handler: async (p) => {
        ...
        try {
          ...
          if (p.append) fs.appendFileSync(fp, String(p.content));
          else fs.writeFileSync(fp, String(p.content));
      ...
      handler: async (p) => {
        ...
        try {
          const entries = fs.readdirSync(dp, { withFileTypes: true }).map(e => {
            try { const s = fs.statSync(path.join(dp, e.name)); return { name: e.name, type: e.isDirectory() ? "dir" : "file", size: s.size, mtime: s.mtime.toISOString() }; }
            catch { return { name: e.name, type: "unknown", size: 0 }; }
          });
    ...
    },
    {
      ...
      handler: async (p) => {
        ...
        if (p.priority) args.push(`-p`, String(p.priority));
        if (p.since) args.push(`--since`, String(p.since));
    ...
    },
    {
      ...
      handler: async (p) => {
        ...
        const parts: string[] = [];
        if (d === "all" || d === "interfaces") parts.push("=== Interfaces ===\n" + runShell("ip -br addr"));
        if (d === "all" || d === "ports") parts.push("=== Listening Ports ===\n" + runShell("ss -tlnp"));
        if (d === "all" || d === "dns") parts.push("=== DNS ===\n" + runShell("cat /etc/resolv.conf 2>/dev/null"));
        if (d === "all" || d === "routes") parts.push("=== Routes ===\n" + runShell("ip route"));
        return parts.join("\n\n");
      },
    },
    ...
    // ═══════════════════════════════════════════════════════════════
    ...daemonProxy("wallet_create", "Create crypto wallet (ETH or SOL)", keyd, "POST", "/wallet/create",
      { chain: { type: "string", description: "ethereum|solana" }, label: { type: "string" } }, ["chain"]),
    ...daemonProxy("wallet_list", "List all wallets", keyd, "GET", "/wallet/list", {}),
    ...daemonProxy("wallet_sign", "Sign a payload with wallet key", keyd, "POST", "/wallet/sign",
      { wallet_id: { type: "string" }, payload: { type: "string", description: "Hex-encoded payload" } }, ["wallet_id", "payload"]),
    ...daemonProxy("wallet_send", "Build a signed transfer transaction", keyd, "POST", "/wallet/send",
      { wallet_id: { type: "string" }, to: { type: "string" }, amount: { type: "string" } }, ["wallet_id", "to", "amount"]),
    ...daemonProxy("wallet_delete", "Delete a wallet", keyd, "POST", "/wallet/delete",
      { wallet_id: { type: "string" } }, ["wallet_id"]),
    ...daemonProxy("wallet_receipt", "Get transaction receipt from ledger", keyd, "GET", "/wallet/receipt",
      { wallet_id: { type: "string" }, limit: { type: "number" } }),
    ...daemonProxy("wallet_build_tx", "Build a custom transaction", keyd, "POST", "/wallet/build_tx",
      { wallet_id: { type: "string" }, chain: { type: "string" }, tx_type: { type: "string" }, to: { type: "string" }, amount: { type: "string" }, chain_params: { type: "object" } }, ["wallet_id", "chain", "to", "amount"]),

    // ═══════════════════════════════════════════════════════════════
    // SAFESWITCH (5 via watch)
    // ═══════════════════════════════════════════════════════════════
    ...daemonProxy("safe_switch_begin", "Begin a SafeSwitch deploy transaction with health checks", watch, "POST", "/switch/begin",
      { plan: { type: "string" }, ttl_secs: { type: "number" }, health_checks: { type: "array", items: { type: "object" } } }, ["plan"]),
    ...daemonProxy("safe_switch_list", "List all SafeSwitch sessions", watch, "GET", "/switch/list", {}),
    ...daemonProxy("safe_switch_status", "Get SafeSwitch session status", watch, "GET", "/switch/status",
      { id: { type: "string" } }, ["id"], (p) => `/switch/status/${p.id}`),
    ...daemonProxy("safe_switch_commit", "Commit a SafeSwitch session", watch, "POST", "/switch/commit",
      { id: { type: "string" } }, ["id"], (p) => `/switch/commit/${p.id}`),
    ...daemonProxy("safe_switch_rollback", "Rollback a SafeSwitch session", watch, "POST", "/switch/rollback",
      { id: { type: "string" } }, ["id"], (p) => `/switch/rollback/${p.id}`),

    // ═══════════════════════════════════════════════════════════════
    // WATCHERS (2 via watch)
    // ═══════════════════════════════════════════════════════════════
    ...daemonProxy("watcher_add", "Add a health watcher", watch, "POST", "/watcher/add",
      { name: { type: "string" }, check: { type: "object" }, interval_secs: { type: "number" }, actions: { type: "array", items: { type: "object" } } }, ["name", "check"]),
    ...daemonProxy("watcher_list", "List all health watchers", watch, "GET", "/watcher/list", {}),

    // ═══════════════════════════════════════════════════════════════
    // ROUTINES (3 via routines)
    // ═══════════════════════════════════════════════════════════════
    ...daemonProxy("routine_add", "Add a background automation routine", routines, "POST", "/routine/add",
      { name: { type: "string" }, trigger: { type: "object" }, action: { type: "object" } }, ["name", "trigger", "action"]),
    ...daemonProxy("routine_list", "List all routines", routines, "GET", "/routine/list", {}),
    ...daemonProxy("routine_trigger", "Manually trigger a routine", routines, "POST", "/routine/trigger",
      { id: { type: "string" } }, ["id"], (p) => `/routine/trigger/${p.id}`),

    // ═══════════════════════════════════════════════════════════════
    // IDENTITY + RECEIPTS (4 via agentd)
    // ═══════════════════════════════════════════════════════════════
    ...daemonProxy("agent_card", "Get the agent's EIP-8004 Agent Card", agentd, "GET", "/agent/card", {}),
    ...daemonProxy("receipt_list", "List audit receipts", agentd, "GET", "/receipts",
      { type: { type: "string" }, since: { type: "string" }, limit: { type: "number" } }),
    ...daemonProxy("incident_create", "Create an incident workspace", agentd, "POST", "/incident/create",
      { name: { type: "string" } }, ["name"]),
    ...daemonProxy("incident_step", "Add a step to an incident", agentd, "POST", "/incident/step",
      { id: { type: "string" }, action: { type: "string" }, result: { type: "string" } }, ["id", "action"], (p) => `/incident/${p.id}/step`),

    // ═══════════════════════════════════════════════════════════════
    // VOICE (5 via voice daemon)
    // ═══════════════════════════════════════════════════════════════
    ...daemonProxy("voice_status", "Voice pipeline status", voice, "GET", "/voice/status", {}),
    ...daemonProxy("voice_speak", "Text-to-speech (local piper)", voice, "POST", "/voice/speak",
      { text: { type: "string" } }, ["text"]),
    ...daemonProxy("voice_transcribe", "Speech-to-text (local whisper)", voice, "POST", "/voice/transcribe",
      { audio_path: { type: "string" } }, ["audio_path"]),
    ...daemonProxy("voice_record", "Record audio from microphone", voice, "POST", "/voice/record",
      { duration_secs: { type: "number" }, transcribe: { type: "boolean" } }),
    ...daemonProxy("voice_listen", "Toggle continuous voice listening", voice, "POST", "/voice/listen",
    ...
    ...daemonProxy("mesh_invite_create", "Create a mesh invite code", mesh, "POST", "/invite/create",
      { ttl_secs: { type: "number" } }),
    ...daemonProxy("mesh_invite_accept", "Accept a mesh invite", mesh, "POST", "/invite/accept",
      { invite_code: { type: "string" } }, ["invite_code"]),
    ...daemonProxy("mesh_peers", "List mesh peers", mesh, "GET", "/peers", {}),
    ...daemonProxy("mesh_peer_send", "Send a message to a mesh peer", mesh, "POST", "/peer/send",
      { id: { type: "string" }, message: { type: "object" } }, ["id", "message"], (p) => `/peer/${p.id}/send`),
    ...daemonProxy("mesh_peer_disconnect", "Disconnect a mesh peer", mesh, "DELETE", "/peer/disconnect",
      { id: { type: "string" } }, ["id"], (p) => `/peer/${p.id}`),
    ...daemonProxy("mesh_health", "Mesh network health", mesh, "GET", "/health", {}),
    ...daemonProxy("mesh_room_create", "Create a mesh chat room", mesh, "POST", "/room/create",
      { name: { type: "string" }, peer_ids: { type: "array", items: { type: "string" } } }, ["name"]),
    ...daemonProxy("mesh_room_join", "Join a mesh room", mesh, "POST", "/room/join",
      { room_id: { type: "string" } }, ["room_id"]),
    ...daemonProxy("mesh_room_send", "Send message to a mesh room", mesh, "POST", "/room/send",
      { room_id: { type: "string" }, message: { type: "string" } }, ["room_id", "message"]),
    ...daemonProxy("mesh_room_history", "Get mesh room message history", mesh, "GET", "/room/history",
      { room_id: { type: "string" }, limit: { type: "number" } }, ["room_id"], (p) => `/room/${p.room_id}/history`),

    // ═══════════════════════════════════════════════════════════════
    // MCP SERVER MANAGEMENT (4 via mcpd)
    // ═══════════════════════════════════════════════════════════════
    ...daemonProxy("mcp_servers", "List MCP servers", mcpd, "GET", "/servers", {}),
    ...daemonProxy("mcp_server_start", "Start an MCP server", mcpd, "POST", "/server/start",
      { name: { type: "string" } }, ["name"], (p) => `/server/${p.name}/start`),
    ...daemonProxy("mcp_server_stop", "Stop an MCP server", mcpd, "POST", "/server/stop",
      { name: { type: "string" } }, ["name"], (p) => `/server/${p.name}/stop`),
    ...daemonProxy("mcp_server_restart", "Restart an MCP server", mcpd, "POST", "/server/restart",
      { name: { type: "string" } }, ["name"], (p) => `/server/${p.name}/restart`),
    ...
    },
    {
      ...
      handler: async () => {
        ...
        units.forEach(u => { try { child_process.execSync(`systemctl stop ${u} 2>/dev/null`); } catch {} });
    ...
    ...daemonProxy("teach_observations", "Recent system observations", teachd, "GET", "/observations",
      { source: { type: "string" }, since: { type: "string" }, limit: { type: "number" } }),
    ...daemonProxy("teach_patterns", "Detected system patterns", teachd, "GET", "/patterns",
      { type: { type: "string" }, min_confidence: { type: "number" } }),
    ...daemonProxy("teach_knowledge", "Knowledge documents", teachd, "GET", "/knowledge",
      { category: { type: "string" }, tag: { type: "string" }, limit: { type: "number" } }),
    ...daemonProxy("teach_knowledge_create", "Create a knowledge document", teachd, "POST", "/knowledge/create",
      { title: { type: "string" }, category: { type: "string" }, content: { type: "string" }, tags: { type: "array", items: { type: "string" } } }, ["title", "content"]),
    ...daemonProxy("teach_context", "Inject relevant knowledge into prompt", teachd, "POST", "/teach",
      { context: { type: "string" } }, ["context"]),
    ...daemonProxy("teach_optimize_suggest", "Get optimization suggestions", teachd, "POST", "/optimize/suggest", {}),
    ...daemonProxy("teach_optimize_apply", "Apply an optimization via SafeSwitch", teachd, "POST", "/optimize/apply",
      { id: { type: "string" } }, ["id"], (p) => `/optimize/apply/${p.id}`),
    ...daemonProxy("teach_skill_candidates", "List auto-detected skill candidates", teachd, "GET", "/skills/candidates",
      { status: { type: "string" }, limit: { type: "number" } }),
    ...daemonProxy("teach_skill_generate", "Generate SKILL.md from a candidate", teachd, "POST", "/skills/generate",
      { id: { type: "string" } }, ["id"], (p) => `/skills/generate/${p.id}`),
    ...daemonProxy("teach_skill_promote", "Promote a skill to auto-activation", teachd, "POST", "/skills/promote",
      { id: { type: "string" } }, ["id"], (p) => `/skills/promote/${p.id}`),
    ...daemonProxy("teach_observe_action", "Log an agent action for skill learning", teachd, "POST", "/observe/action",
      { tool: { type: "string" }, params: { type: "object" }, result_summary: { type: "string" }, context: { type: "string" }, session_id: { type: "string" }, success: { type: "boolean" } }, ["tool"]),
    ...daemonProxy("teach_skill_execution", "Log a skill execution outcome", teachd, "POST", "/skills/execution",
      { skill_name: { type: "string" }, outcome: { type: "string" }, session_id: { type: "string" }, notes: { type: "string" } }, ["skill_name", "outcome"]),
    ...daemonProxy("teach_skill_detect", "Manually trigger skill sequence detection", teachd, "POST", "/skills/detect", {}),

    // ═══════════════════════════════════════════════════════════════
    // APPROVAL (4 via agentd)
    // ═══════════════════════════════════════════════════════════════
    ...daemonProxy("approval_request", "Request approval for a command", agentd, "POST", "/approval/request",
      { command: { type: "string" }, reason: { type: "string" } }, ["command"]),
    ...daemonProxy("approval_pending", "List pending approvals", agentd, "GET", "/approval/pending", {}),
    ...daemonProxy("approval_approve", "Approve a pending request", agentd, "POST", "/approval/approve",
      { id: { type: "string" } }, ["id"], (p) => `/approval/${p.id}/approve`),
    ...daemonProxy("approval_check", "Check approval status", agentd, "GET", "/approval/check",
      { id: { type: "string" } }, ["id"], (p) => `/approval/${p.id}`),

    // ═══════════════════════════════════════════════════════════════
    // SANDBOX + CAPABILITY (2 via agentd)
    // ═══════════════════════════════════════════════════════════════
    ...daemonProxy("sandbox_exec", "Execute in sandboxed environment", agentd, "POST", "/sandbox/exec",
      { command: { type: "string" }, ring: { type: "number", description: "1 (approved) or 2 (untrusted)" }, capabilities: { type: "array", items: { type: "string" } }, timeout_secs: { type: "number" } }, ["command"]),
    ...daemonProxy("capability_mint", "Mint a capability token", agentd, "POST", "/capability/mint",
      { granted_to: { type: "string" }, permissions: { type: "array", items: { type: "string" } }, ttl_secs: { type: "number" } }, ["granted_to", "permissions"]),

    // ═══════════════════════════════════════════════════════════════
    // FLEET (4 via watch)
    // ═══════════════════════════════════════════════════════════════
    ...daemonProxy("fleet_propose", "Propose a fleet-wide SafeSwitch", watch, "POST", "/fleet/propose",
      { plan: { type: "string" }, peer_ids: { type: "array", items: { type: "string" } }, health_checks: { type: "array", items: { type: "object" } }, quorum_percent: { type: "number" }, timeout_secs: { type: "number" } }, ["plan", "peer_ids"]),
    ...daemonProxy("fleet_status", "Fleet switch status", watch, "GET", "/fleet/status",
      { id: { type: "string" } }, ["id"], (p) => `/fleet/status/${p.id}`),
    ...daemonProxy("fleet_vote", "Vote on a fleet switch", watch, "POST", "/fleet/vote",
      { id: { type: "string" }, peer_id: { type: "string" }, approve: { type: "boolean" }, reason: { type: "string" } }, ["id", "peer_id", "approve"], (p) => `/fleet/vote/${p.id}`),
    ...daemonProxy("fleet_rollback", "Rollback a fleet switch", watch, "POST", "/fleet/rollback",
      { id: { type: "string" } }, ["id"], (p) => `/fleet/rollback/${p.id}`),
    ...
    // ═══════════════════════════════════════════════════════════════
    {
      ...
      handler: async (p) => {
        ...
        if (p.working_dir) args.push(`--working-directory=${p.working_dir}`);
        if (p.memory_max) args.push(`--property=MemoryMax=${p.memory_max}`);
        if (p.cpu_quota) args.push(`--property=CPUQuota=${p.cpu_quota}`);
        if (p.env && typeof p.env === "object") { for (const [k, v] of Object.entries(p.env as Record<string, string>)) args.push(`--setenv=${k}=${v}`); }
        args.push("--", String(p.command));
        if (Array.isArray(p.args)) (p.args as string[]).forEach(a => args.push(String(a)));
    ...
    },
    {
      ...
      handler: async () => {
        const apps = loadApps();
        const list = Object.values(apps).map((a: any) => {
          const unit = APP_PREFIX + a.name;
          let active = "unknown";
          try { active = child_process.execSync(`systemctl is-active ${unit} 2>/dev/null`, { encoding: "utf-8" }).trim(); } catch { active = "inactive"; }
          return { ...a, active };
        });
    ...
    },
    {
      ...
      handler: async (p) => {
        ...
        const apps = loadApps();
        if (apps[name]) { apps[name].status = "stopped"; saveApps(apps); }
    ...
    },
    {
      ...
      handler: async (p) => {
        ...
        runShell(`systemctl stop ${APP_PREFIX}${name} 2>/dev/null`);
        const apps = loadApps();
        if (apps[name]) { apps[name].status = "removed"; saveApps(apps); }
...
function daemonProxy(
  name: string,
  description: string,
  daemon: (m: string, p: string, b?: unknown) => Promise<string>,
  method: string,
  defaultPath: string,
  props: Record<string, any>,
  required?: string[],
  pathFn?: (p: Record<string, unknown>) => string,
): ToolDef[] {
  return [{
    name,
    description,
    inputSchema: { type: "object" as const, properties: props, required: required || [] },
