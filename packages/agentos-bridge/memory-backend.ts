/**
 * AgentOS Memory Backend for OpenClaw.
 *
...
export class AgentOSMemoryBackend {
  ...
  async search(query: string, options: SearchOptions = {}): Promise<MemoryChunk[]> {
    try {
      ...
      console.error("[agentos-memory] Recall failed, continuing without memories:", err);
      return [];
    }
  }
  ...
  async index(content: string, metadata: IndexMetadata = {}): Promise<void> {
    try {
      ...
      console.error("[agentos-memory] Index failed:", err);
    }
  }
  ...
  async flush(context: CompactionContext): Promise<void> {
    try {
      ...
      console.error("[agentos-memory] Flush failed:", err);
    }
  }
}
