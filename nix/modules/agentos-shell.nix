# AgentOS Shell — Kiosk-mode desktop shell
...
let
  cfg = config.services.agentos.shell;
  gatewayPort = config.services.agentos.openclaw.port;
  ...
  '';
in {
  options.services.agentos.shell = {
    enable = mkEnableOption "AgentOS kiosk shell (Sway + Firefox fullscreen)";
