...
> **Inbound x402 is live on testnet.** spawn.os.moda accepts USDC payments via x402 for server
> provisioning (see [SPAWN-API.md](SPAWN-API.md)). As of April 2026 the public x402 facilitators
> (`x402.org/facilitator`, `facilitator.x402.rs`) only support testnets — **Base Sepolia**
> (`eip155:84532`) and **Solana Devnet**. The spawn API runs in `NETWORK_MODE=testnet` so the
> agent-to-agent flow stays honest. Mainnet will re-enable once a production facilitator ships
> or we self-host one.
>
> This document describes the **outbound** side — the OS autonomously paying for external APIs
> through osmoda-egress + osmoda-keyd.
...
Supports: USDC on Base (EVM) and Solana (SVM), plus any EIP-3009 token on any EVM chain.
Cost: $0.001–$0.01 per call typical. Near-zero gas (ERC-3009 is gasless, Solana fees ~$0.0005).
...
- **osmoda-egress** — the HTTP proxy every daemon and Tier 1/2 app routes through
...
```
osmoda-mesh, routines, or Tier 1 app
  │ HTTP request
  ▼
